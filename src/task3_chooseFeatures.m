...
% === Features prioritaires imposées ===
prioritary_features = {'spectral_rolloff_mean', 'mfcc_1_mean', 'spectral_centroid_mean', 'tempo'};
prioritary_to_use = intersect(prioritary_features, all_features);
n_prioritary_needed = 3;
...
% Étape 1 : Forcer la sélection de 3 parmi les 4 features prioritaires
forced_features = {};
for i = 1:n_prioritary_needed
    ...
    fprintf('Feature prioritaire %d : %s — Rayleigh = %.4f\n', ...
        i, best_feature, best_score);
end

% Étape 2 : Sélection libre de la feature restante
best_score = -inf;
best_feature = '';
...
fprintf('Dernière feature libre : %s — Rayleigh = %.4f\n', ...
    best_feature, best_score);

% === Résultat final ===
fprintf('\nFeatures sélectionnées :\n');
disp(selected_features');
fprintf('Historique des coefficients de Rayleigh :\n');
disp(rayleigh_history);

%% === Fonction de calcul du coefficient de Rayleigh ===
